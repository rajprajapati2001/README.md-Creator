...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (splashScreen) {
      ...
      setTimeout(() => {
        // 2. Trigger the fade-out CSS transition
        splashScreen.style.opacity = '0';
        splashScreen.style.visibility = 'hidden';
        
        // 3. Remove it from the DOM after the 0.5s transition finishes
        setTimeout(() => {
          splashScreen.remove();
        }, 500);
  ...
  // State: Is Sidebar open or closed (Hide/Show Slide bar) - starts closed on mobile (<640px) to prevent layout flicker
...
root.render(<App />);`
  ...
  return (
    ...
    {
      id: "7",
      name: "readme-icon.svg",
      path: "/assets/readme-icon.svg",
      isFolder: false,
      parentId: "5",
      content: "IMAGE_MOCK_DATA_URL:/assets/readme-icon.svg"
    }
  ]);
...
`;
  ...
  const renderMarkdownBlockquote = ({ children }: any) => {
    ...
    if (React.isValidElement(firstChild) && firstChild.type === 'p') {
      ...
      if (typeof firstTextNode === 'string') {
        const match = firstTextNode.match(/^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]/i);
        if (match) {
          const alertType = match[1].toUpperCase();
          const restText = firstTextNode.replace(/^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*/i, '');

          // Re-create the paragraph with the tag removed
          ...
          return (
            <div className={`gh-alert gh-alert-${alertType.toLowerCase()}`}>
              <div className="flex items-center gap-2 font-semibold text-xs mb-1.5 uppercase tracking-wider alert-title select-none">
                {alertType === 'NOTE' && <Info className="w-4 h-4" />}
                {alertType === 'TIP' && <Lightbulb className="w-4 h-4" />}
                {alertType === 'IMPORTANT' && <AlertCircle className="w-4 h-4" />}
                {alertType === 'WARNING' && <AlertTriangle className="w-4 h-4" />}
                {alertType === 'CAUTION' && <Ban className="w-4 h-4" />}
                <span>{alertType}</span>
              </div>
              <div className="alert-content text-sm leading-relaxed">
                {updatedParagraph}
                {content.slice(1)}
              </div>
            </div>
    ...
    return (
      <blockquote className="border-l-4 pl-4 italic my-4 border-[#30363D]">
        {children}
      </blockquote>
    );
  };
    ...
    <div className="flex flex-col h-screen w-screen bg-[#0D1117] text-[#C9D1D9] font-sans overflow-hidden select-none">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 text-xs font-bold rounded border transition-all cursor-pointer ${syncScroll
      ...
      {notification && (
        <div className="fixed left-1/2 md:top-2 top-4 z-50 -translate-x-1/2 w-auto max-w-max px-4">
          <div className={`flex items-center gap-2 rounded-full border p-1.5 pl-3 pr-2 shadow-[0_10px_30px_rgba(0,0,0,0.15)] transition-all duration-300 ${notification.type === "success"
        ...
        >
          ...
          <div className="grid grid-cols-3 border-b border-[#30363D] bg-[#161B22] text-[#8B949E] text-center shrink-0">
            <button
              ...
              className={`py-3.5 text-[10px] uppercase font-black tracking-wider border-b-2 transition-all cursor-pointer ${activeTab === "toolbox" ? "border-blue-500 text-white bg-[#0D1117]" : "border-transparent hover:text-white hover:bg-[#161B22]"
                }`}
            ...
            <button
              ...
              className={`py-3.5 text-[10px] uppercase font-black tracking-wider border-b-2 transition-all cursor-pointer ${activeTab === "workspace" ? "border-blue-500 text-white bg-[#0D1117]" : "border-transparent hover:text-white hover:bg-[#161B22]"
                }`}
            ...
            <button
              ...
              className={`py-3.5 text-[10px] uppercase font-black tracking-wider border-b-2 transition-all cursor-pointer ${activeTab === "templates" ? "border-blue-500 text-white bg-[#0D1117]" : "border-transparent hover:text-white hover:bg-[#161B22]"
                }`}
                    ...
                    {TECH_ICONS.map(tech => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-1 rounded text-2xs truncate font-medium text-center transition-all cursor-pointer ${isSelected
                  ...
                  <div className="p-3 bg-[#161B22] border border-[#30363D] rounded-lg text-xs space-y-2.5">
                    ...
                    <div className="space-y-1">
                      ...
                      <div
                        ...
                        className={`border-2 border-dashed rounded-lg p-5 text-center transition-all relative z-10 ${typeof isDragging !== 'undefined' && isDragging
...
Use this text area to craft premium markdown documentation..."
          ...
          <div
            ...
            className={`w-full sm:w-1/2 min-h-0 flex flex-col transition-all duration-200 ${previewTheme === "light" ? "bg-white text-[#1F2328]" : "bg-[#0d1117] text-[#c9d1d9]"
              }`}>
            <div className={`h-10 flex items-center justify-between px-4 shrink-0 text-xs border-b ${previewTheme === "light"
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className={`p-1.5 rounded transition-all cursor-pointer border ${previewTheme === "light"
            ...
            >
              <div className={`max-w-2xl mx-auto p-4 rounded-lg transition-all ${previewTheme === "light" ? "gh-markdown-light" : "gh-markdown-dark"
                }`}>
                {suspendPreview ? (
      ...
      <footer className="h-7 border-t border-[#30363D] bg-[#161B22] flex items-center justify-between px-4 shrink-0 text-[10px] text-[#8B949E] z-10">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 text-gray-400 hover:text-white transition-colors group p-1 rounded hover:bg-[#30363D]"
            title="About this website"
          >
            <Info className="w-4 h-4 group-hover:text-blue-400 transition-colors" />
            <span className="font-semibold uppercase tracking-wider text-[9px] hidden sm:inline">About</span>
          </button>
        </div>
      </footer>
      ...
      {isAboutModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-[#1C2128] border border-[#30363D] rounded-xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col relative animate-fade-in">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-[#30363D] bg-[#22272E]">
              ...
              <button
                ...
                className="text-gray-400 hover:text-white transition-colors p-1 rounded-md hover:bg-[#30363D]"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 flex flex-col items-center text-center">
              <div className="w-25 h-25 flex items-center justify-center mb-4 ">
                <img src="/assets/readme-icon-nobg.svg" alt="Icon" className="w-25 h-25" />
              </div>

              <h4 className="text-white font-bold text-lg mb-2">README Creator</h4>
              <p className="text-gray-400 text-sm mb-6 leading-relaxed">
                A powerful, interactive tool to craft the perfect README for your open-source projects.
              </p>

              <div className="w-full border-t border-[#30363D] pt-4 mt-2">
                <h4 className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-3">Developed by</h4>
                <div className="flex items-center justify-center gap-6">
                  <a
                    href="https://github.com/rajprajapati2001"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex flex-col items-center gap-2 group cursor-pointer"
                  >
                    <div className="relative w-14 h-14 transition-all duration-300 shadow-sm group-hover:-translate-y-1">
                      <img
                        src="https://github.com/rajprajapati2001.png"
                        alt="Raj Prajapati"
                        className="w-full h-full rounded-full border-2 border-[#444C56] group-hover:border-blue-500 object-cover transition-colors"
                      />
                      <div className="absolute -bottom-1 -right-1 bg-[#1C2128] rounded-full p-[3px]">
                        <div className="bg-[#2D333B] rounded-full p-1 group-hover:bg-blue-500 transition-colors">
                          <Github className="w-3 h-3 text-gray-200 group-hover:text-white" />
                        </div>
                      </div>
                    </div>
                    <span className="text-xs text-wrap text-gray-400 group-hover:text-gray-200 font-medium mt-1">Raj Prajapati</span>
                  </a>

                  <a
                    href="https://github.com/Devendra2553"
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  >
                    <div className="relative w-14 h-14 transition-all duration-300 shadow-sm group-hover:-translate-y-1">
                      <img
                        src="https://github.com/Devendra2553.png"
                        alt="Devendra Chauhan"
                        className="w-full h-full rounded-full border-2 border-[#444C56] group-hover:border-blue-500 object-cover transition-colors"
                      />
                      <div className="absolute -bottom-1 -right-1 bg-[#1C2128] rounded-full p-[3px]">
                        <div className="bg-[#2D333B] rounded-full p-1 group-hover:bg-blue-500 transition-colors">
                    ...
                    <span className="text-xs text-wrap text-gray-400 group-hover:text-gray-200 font-medium mt-1">Devendra Chauhan</span>
                  </a>
                </div>
              </div>
            </div>
